...
class PopUp_Domination {
	...
	*/
	...
	function popup_domination_dashboard_function() {
		...
		try{
			$rss = new DOMDocument();
			$return = @$rss->load('http://www.popupdomination.com/blog/feed/');
			if (empty($return))
				throw new Exception("URL File-access is disabled on the server.");
		...
		//if it didnt work then manually create these -> should be updated when possible
		//temp solution to those with URL file access disabled -> TODO find permanent solution
		if (!$success){
			$feed[] = array("title" => "Internet Marketing Info Graphic Reveals How Neil Patel Runs His Online Business!",
							"desc" => "Something very exciting is coming soon. One lucky commenter will get access to it for FREE. All you have to do is comment below telling us, what would help your online business the most right now. Make sure to come back 3rd July to see our industry changing new product!",
							"link" => "http://www.popupdomination.com/blog/internet-marketing-info-graphic-reveals-how-neil-patel-runs-his-online-business/",
							"date" => "Monday June 25, 2012");
							
			$feed[] = array("title" => "Multi-lingual Support",
							"desc" => "After receiving multiple requests for support for multiple languages, we realized that this is clearly something our users want and need. We have listened, we have delivered. PopUp Domination 3.3.4.3 provides multi-lingual support for those of who wish to create personalized pop ups with your OWN material in your OWN language. On top of this,[...]",
							"link" => "http://www.popupdomination.com/blog/multi-lingual-support/",
							"date" => "Thursday May 31, 2012");
			
			$feed[] = array("title" => "PD Analytics Fix",
							"desc" => "Today we released an update for PopUp Domination to fix some of the the little bugs that have been affecting our users. Some of our users have been unhappy that their Analytics were not working correctly and therefore, could not accurately tell what was working for them and what wasn?t. Well today we hope to[...]",
							"link" => "http://www.popupdomination.com/blog/popupdomination-analytics-fix/",
							"date" => "Tuesday May 01, 2012");
			$feed[] = array("title" => "PD Update",
							"desc" => "So here is an update on our progress here at PopUp Domination. Over the past few weeks we have been looking at what we have been doing right and what we haven?t been doing so well. As our loyal customers, we want to reward you with the best product we can offer. So many people[...]",
							"link" => "http://www.popupdomination.com/blog/pd-update-april-2012/",
							"date" => "Monday April 23, 2012");
			$feed[] = array("title" => "Stand-Alone v3.4.5",
							"desc" => "There was a bug within the A/B Testing functionality which was failing to update the analytics every time a user was converted. This affected the Stand-Alone version 3.4 and earlier. This error has now been corrected.",
							"link" => "http://www.popupdomination.com/blog/stand-alone-v3-4-5/",
  ...
  function popdom_inpost($content){
    if ($this->show_anim == "inpost" && is_singular() ) {
	...
	*/
	function version_3_5_upgrade(){
		...
		//$drop = "DROP TABLE `".$wpdb->prefix."popdom_analytics`";
		//$wpdb->query($drop);
	}
