...
class Dropbox_OAuth_WordPress extends Dropbox_OAuth {
	...
	public function fetch($uri, $arguments = array(), $method = 'GET', $httpHeaders = array()) {
		...
		if(is_array($arguments)) {
			...
			list($oauth_uri) = explode('?', $uri);
			parse_str(parse_url($uri, PHP_URL_QUERY), $oauth_arguments);
		}
		
		$requestParams['method'] = $method;
		$oAuthHeader = $this->getOAuthHeader($oauth_uri, $oauth_arguments, $method);
		...
		$requestParams['sslverify'] = false;
		
		// 20 Minutes
		$requestParams['timeout'] = 60 * 20;

		$request = new WP_Http;

		$result = $request->request($uri, $requestParams);

		return is_wp_error($result) ? array() : array('httpStatus' => $result['response']['code'], 'body' => $result['body']);
	}
